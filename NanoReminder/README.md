...
支持表情：`calm` / `happy` / `grateful` / `confused` / `ask` / `panic` / `shocked`。
...
也可以安装用户级 hooks，让所有 Claude Code 对话结束时自动把最终回复镜像成 Nano 弹窗，并把 ask/权限确认拦截成 Nano 弹窗，不依赖 MCP：
...
hook 会读取最终回复末尾的 `<!-- nano-mood:happy -->` 这类隐藏标记选择表情，并在已经发过 Nano 通知时跳过，避免重复弹窗。ask/权限确认使用专门的 `ask` 疑惑头像。
